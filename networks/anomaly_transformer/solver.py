...
class AnomalyTransformer(object):
    ...
    def predict_single_point(self, input_data):
        """
        对单个时间点进行异常分数预测。
        Args:
            input_data (torch.Tensor): 单个时间点的数据，形状应为 (1, window_size, input_c)。
        Returns:
            float: 该时间点的异常分数。
        """
        ...
        with torch.no_grad():
            ...
            cri = metric * loss
            anomaly_score = cri.detach().cpu().numpy().mean()
...
def train_model(params, train_loader, valid_loader):
    """
    训练 AnomalyTransformer 模型。
    Args:
        params (dict): 包含模型参数的字典。
        train_loader (torch.utils.data.DataLoader): 训练数据加载器。
        valid_loader (torch.utils.data.DataLoader): 验证数据加载器。
    Returns:
        AnomalyTransformer: 训练好的模型实例。
    """
